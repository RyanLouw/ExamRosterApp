...
public interface IAppRepository { Task<List<Teacher>> GetTeachersAsync(); Task<List<ExamDutySlot>> GetSlotsAsync(); Task<List<TeacherUnavailableSlot>> GetUnavailableAsync(); Task<List<DutyAssignment>> GetAssignmentsAsync(); Task SaveAssignmentsAsync(IEnumerable<DutyAssignment> a); Task ClearAssignmentsAsync(); Task SaveChangesAsync(); }
